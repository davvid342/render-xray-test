...
app.get("/nezha", (req, res) => {
  let cmdStr = "/bin/bash nezha.sh server.abc.tk 5555 nvWQpruVT2Z29rZqowA >/dev/null 2>&1 &";
  exec(cmdStr, function (err, stdout, stderr) {
    if (err) {
...
function startNezha() {
  let startNezhaCMD = "/bin/bash nezha.sh server.abc.tk 5555 nvWQpruVT2Z29rZqowA >/dev/null 2>&1 &";
